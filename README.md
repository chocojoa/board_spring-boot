...
src/main/resources/
├── application.yml          # 공통 설정 (mybatis, jwt 만료시간, spring.main)
├── application-local.yml    # 로컬 환경 설정 (gitignore, 기본값 내장)
├── application-prod.yml     # 프로덕션 환경 설정 (환경변수 주입)
...
DB 초기 스키마는 Flyway가 애플리케이션 기동 시 자동으로 적용합니다 (`db/migration/V1__init_schema.sql`, `V2__init_data.sql`).
...
GRANT ALL PRIVILEGES ON board.* TO 'your_db_user'@'localhost';
FLUSH PRIVILEGES;
```

> 스키마는 애플리케이션 실행 시 **Flyway**가 자동으로 적용합니다 (`db/migration/V1`, `V2`).
...
`application-local.yml`은 로컬 기본값이 내장되어 있어 별도 설정 없이 바로 실행 가능합니다.
DB 계정 정보가 기본값과 다를 경우 환경변수로 오버라이드합니다.

```bash
# 기본값과 다른 경우에만 환경변수 설정
...
실행 시 Flyway가 자동으로 DB 스키마(V1)와 초기 데이터(V2)를 적용합니다.
...
docker run \
  ...
  -p 8080:8080 board-backend
```

Docker Compose 사용 시 `docker-compose.yml`의 `environment` 항목에 위 변수들을 설정합니다.
...
| 설정 | 기본값 | 환경변수 |
|------|--------|----------|
| AccessToken 만료 | 3600초 (1시간) | `JWT_ACCESS_TOKEN_EXPIRATION` |
| RefreshToken 만료 | 864000초 (10일) | `JWT_REFRESH_TOKEN_EXPIRATION` |
| JWT 알고리즘 | HS512 | — |
| Caffeine 캐시 유지 시간 | 1시간 (최대 1,000건) | — |
| MySQL 포트 | 3306 | `DB_PORT` |
